...
if [ -n "${DOGTAG_DOWNLOAD_BASE:-}" ]; then
  ...
  if [ -z "$version" ]; then
    ...
    version="$(fetch "$releases_url" - \
      | grep '"tag_name"' \
      | head -n 1 \
      | sed 's/.*"tag_name"[^"]*"\([^"]*\)".*/\1/')" \
      || err "could not query $releases_url for the newest release"
    [ -n "$version" ] || err "could not parse a tag_name from $releases_url (no releases yet?)"
  fi
  version="${version#v}"
...
cp "$tmp/dogtag" "$INSTALL_DIR/dogtag" || err "could not write $INSTALL_DIR/dogtag"
chmod +x "$INSTALL_DIR/dogtag"
