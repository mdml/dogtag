...
//!   present       yes
//!   version       3 (current; supported 1..=3)
//! types           4 declared
...
mod tests {
    ...
    fn an_unusable_contract_still_reports_the_root_and_the_record() {
        ...
        let rendered = text_of(&tree, Body::new("contract_version = 4\n"), RECORD);
        let reason = "the contract declares a version above the supported range 1..=3";
        assert_holds(&rendered, "  present       yes\n");
        assert_holds(&rendered, "  version       4 (too new; supported 1..=3)\n");
        assert_holds(
            &rendered,
    ...
    fn a_version_no_u32_holds_is_reported_as_too_new_rather_than_as_undeclared() {
        ...
        assert_holds(
            &rendered,
            "  version       beyond `0..=4294967295` (too new; supported 1..=3)\n",
        );
        assert_holds(&rendered, "error[compat.contract-too-new]");
    }
    ...
    fn a_missing_contract_says_so_rather_than_leaving_the_version_blank() {
        ...
        assert_holds(
            &rendered,
            "  version       not declared (supported 1..=3)\n",
        );
        assert_holds(
            &rendered,
