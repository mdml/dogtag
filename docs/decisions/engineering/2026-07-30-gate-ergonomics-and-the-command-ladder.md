...
The four recorded divergences, each with its reason in the source: **msrv-build** and **msrv-test** (CI sets `RUSTUP_TOOLCHAIN` because `rust-toolchain.toml` outranks `rustup default`; `cargo +1.85.0` does the same job locally), **osv** (CI runs Google's pinned reusable workflow with its own scanner image; the local binary is pinned separately in `tools.toml` and will drift, so a local scan is a pre-flight and CI is the verdict), and **zizmor** (CI passes the pinned action its settings as inputs; the local command mirrors them as flags).

That last one carries a rule the others do not, learned the expensive way: **a divergence may be mechanical, never a difference in scope.** The local zizmor gate originally audited `.github/workflows` while the CI action defaulted to the whole repository, so `.github/dependabot.yml` was outside the local scope entirely — a `dependabot-cooldown` finding there passed `just gate`, `just check` and both hooks before failing the required check on the first real pull request. Action inputs versus CLI flags is a difference in how a tool is configured; a narrower path is a difference in what was measured, and the second one makes the local gate a weaker instrument wearing the same name. The local command now audits `.` and `scripts/test_gate.py` asserts it.
