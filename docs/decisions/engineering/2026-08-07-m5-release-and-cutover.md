...
- **2026-08-08 — `init`'s packet question follows second-vault onboarding to M10.** The Decision names it "an M7 packet question, where second-vault onboarding is the workflow that would exercise it." [The daily-driver record](2026-08-08-the-beta-is-the-daily-driver.md) inserted three authoring rungs and moved second-vault onboarding to **M10**, so the rung number in that sentence is stale while its reasoning is not: `init` belongs to whichever milestone onboards a second vault, and that is now M10. AGENTS.md generation is unaffected — M6 did not move.
...
- **2026-08-12 — M6 establishes capture use rather than cutting an incumbent over.** Packet-time verification for M6 established that direct `dogtag capture` had not become a real workflow after the release exercises, while the typed-note and attachment paths remain the distinct filing mechanisms the amendments above already excluded. The preceding amendment was right that the MCP server gives capture its first real consumer and wrong that a parallel run would compare two mechanisms: there is still no incumbent capture lane. M6 closes this rolled-forward obligation with real MCP-capture use and an honest no-incumbent receipt; it does not call first adoption a cutover. Binary attachment handling remains M9 residue.
