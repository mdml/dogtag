...
- **2026-08-03 (later) — the record kind is scheduled, and the prefix gap's predicted findings have a severity.** With the deferral reopened by the amendment below, the M3 packet chose: [contract version 2](2026-08-03-contract-version-2.md) carries the `record` kind exactly as this record scopes it — one level, scalar-lattice fields including `enum`, `list` `of = "record"` — so gap one closes when the affected vault migrates. Gap two stands unfixed, as decided; the undeclared-key findings this record's Consequences predicted are minted at `info` severity by [the document-model record](2026-08-03-m3-document-model.md), which is what keeps a corpus in that position passing `check --strict` while the findings stay visible.
