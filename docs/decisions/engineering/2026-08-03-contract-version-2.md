...
- **2026-08-04 (later) — a tag-namespace prefix obeys the declaration-name rule, refused under `contract.declaration-name-invalid`.** The Decision makes `prefix` a literal, non-empty string and says nothing about a `.`; the implementation routes prefixes through the same never-empty, never-dotted rule every other declaration name obeys, because a prefix reaches provenance's dotted key paths and a dotted prefix would break their injectivity — the exact defect the rule was minted for. Ratified as implemented: `prefix = "v1.2/"` is a load error under the standing identifier, not a new one, and the record-field sentence's rule now covers prefixes explicitly. A corpus wanting a dot inside a prefix has no spelling for it, which is recorded as the rule's cost.
