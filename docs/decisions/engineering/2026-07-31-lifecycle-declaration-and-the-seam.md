...
- **2026-08-04 — the no-axis refusal has a name: `note.lifecycle-axis-absent`.** This record made lifecycle filtering against a `none = true` contract "an error diagnostic" without minting its identifier, and all three implementations in the Phase 12 slice-7 trial flagged the gap independently — and minted three different names, including one whole new diagnostic area. Adjudicated with the merged implementation: the refusal is the contract answering a note-domain question — the axis the filter needs is absent from the declarations — so it lives in the existing `note` area as `note.lifecycle-axis-absent`. No per-command or query area is minted; the area lattice stays `note` and `link` until a diagnostic genuinely fits neither.
