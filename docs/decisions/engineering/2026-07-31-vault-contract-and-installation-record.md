...
- Status: accepted (amended 2026-08-01, 2026-08-03, 2026-08-04 — see [Amendments](#amendments))
- Date: 2026-07-31
...
- **2026-08-03 (later) — the bump is scheduled, the frozen-range clause below is corrected, and the version-1 inferences are ratified.** The amendment below closes by saying the supported range is frozen for the beta and there is no `contract_version` to spend — repeating the misreading [the kind-lattice record's](2026-08-03-the-kind-lattice-against-a-real-corpus.md) own amendment retracted the same day: the floor is frozen, the ceiling is not. The M3 packet spends the version: [contract version 2](2026-08-03-contract-version-2.md) carries the tag vocabulary and the record kind, widens the range to `1..=2` only in the change that lands the per-version key sets and default tables this record's first amendment made due, and ratifies the two inferred version-1 readings the third amendment recorded — `[dialect]` is mandatory (in both versions), and version 1's default table is exactly the two implemented literals.

- **2026-08-03 — the lattice's open question is two-sided, and a real corpus asked for an addition first.** The Consequences frame `float` as a kind with no consumer and state that *"evidence that the lattice can shrink has to come from a real corpus."* The first founder vault's hand-authored contract arrived through exactly that channel and reported the opposite: several properties on one identity-bearing type hold records or lists of records, `list` may not nest, and there is no record kind — so those properties are omitted from that contract rather than declared as a `string` the rendering would then instruct an agent to believe. A second, narrower gap has no fix proposed at all: a reserved-prefix namespace owned by a foreign editor is a pattern, and this format declares names. Neither is a defect in the implementation, which enforces the format as specified. The shrink framing stands as still-true and now incomplete; see [the kind lattice against a real corpus](2026-08-03-the-kind-lattice-against-a-real-corpus.md), which scopes the record kind without scheduling it, since the supported range is frozen for the beta and there is no `contract_version` to spend.

- **2026-08-04 — the "two implemented literals" wording above is corrected where it stands, not only in the sibling.** The 2026-08-03 ratification amendment says version 1's default table is "exactly the two implemented literals"; [the contract-version-2 record's amendment](2026-08-03-contract-version-2.md#amendments) already restated it as the one rule it always was — `required = false` at every leaf spelled `required` (property and relationship both, three leaves at version 1), plus `capabilities = []` — and the record-consistency review found this record still carrying the undercounted sentence. The restatement governs; the sentence above is corrected by this entry rather than silently rewritten.
