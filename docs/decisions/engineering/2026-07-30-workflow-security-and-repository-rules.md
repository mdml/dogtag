...
The job is `Workflow security (zizmor)` in [ci.yml](../../../.github/workflows/ci.yml). Locally `just zizmor` runs the same analyzer version with flags mirroring those inputs, **over the same scope** — the repository root, which is what the action audits when given no path. That is not a detail: zizmor collects more than `.github/workflows`, including `.github/dependabot.yml`, and a local run scoped to the workflows directory misses findings CI will raise. It did exactly that once, and the [gate ergonomics ADR](2026-07-30-gate-ergonomics-and-the-command-ladder.md) records the rule that came out of it.
...
- **Dependabot updates wait out a seven-day cooldown.** [dependabot.yml](../../../.github/dependabot.yml) sets `cooldown.default-days: 7` on both ecosystems, matching the quarantine the TypeScript binding applies to npm. It delays *ordinary version updates only* — Dependabot security updates raised against a published advisory are exempt and still arrive immediately, which is the right asymmetry: sitting out a cooldown on a known-vulnerable dependency would invert the point of having one. zizmor's `dependabot-cooldown` audit enforces the floor.
