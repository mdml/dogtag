...
- **2026-08-03 (later) — the question the cutover amendment raised is answered: mis-specified, and the obligation rolls forward.** [beta.md](../../beta.md#required-properties) now carries the roll-forward clause the M3 packet decided: a milestone whose surface turns out to displace nothing — discoverable only by attempting the cutover — rolls its obligation forward rather than waiving it, and its receipt records that nothing moved. M2's cutover box therefore resolves as rolled-forward into M3's three-part cutover ([the M3 release record](2026-08-03-m3-release-and-cutover.md)), and M2 ships with a receipt that says nothing moved. The seven-day machinery this record designed runs for the first time at M3, on the scheduled corpus lint it was built to compare.
