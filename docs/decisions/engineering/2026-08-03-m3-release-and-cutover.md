...
- **2026-08-12 — M3 closes with all three receipts.** Part one records nothing moved and the scheduled `dogtag check --strict` was established fresh, exactly as the 2026-08-05 amendment specifies. Part two moved the seven skills' structural reading and enumeration steps onto installed `dogtag show` and `dogtag list`; their private incumbent mechanisms retire for those steps, while byte-preserving direct reads, alias-index construction, and property sweeps remain named scope differences. Part three moved schema vocabulary and requirement explanation onto `dogtag contract explain`; `pkm classes` retires as schema authority while ownership policy, templates, and born status remain outside the contract's expressive range and therefore remain named consumers of the incumbent data. Both real transfers ran clean from August 5 through August 12 without a report-changing Dogtag fix or a return to the displaced mechanism. `0.1.0-beta.2` had already satisfied criteria 1–10; these receipts satisfy criterion 11.
