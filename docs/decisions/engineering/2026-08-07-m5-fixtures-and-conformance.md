...
- **2026-08-08 — `conforming-contract-loads-with-zero-diagnostics` admits exactly one classification.** The version split this record makes put two of three committed corpora below the format ceiling, and the compatibility classification a below-ceiling corpus earns is a fact about the release reading it rather than a finding about the corpus — so the universal claim would otherwise have been false by this record's own construction. The claim is restated, not weakened by exception: the report carries zero diagnostics *about the corpus* at any severity, and may carry the compatibility classification — exactly that identifier, at `info`, exactly when the declared version is below the ceiling, and nothing else. The allowance cannot absorb a second finding, a different identifier, or a promoted severity, and a corpus at the current version is still held to silence.
- **2026-08-08 — the coverage floors move down to 98.60 line / 98.65 branch, ratified.** The fourth use of the 2026-08-04 harness-dilution precedent, and the outcome this record's own last consequence anticipated. The ten graduated write cases add roughly 390 harness lines whose uncovered remainder is the `?` propagating a failed assertion and the closures building a failure detail — text that executes only when the product under test misbehaves, which a passing suite cannot make happen. What was covered rather than conceded is stated in the baseline beside the numbers: every one of the ten cases runs against a corpus whose catch-all refuses every write and must say what happened, so a case added without a legible refusal fails rather than diluting silently, and each shared helper's detail carries its own named test. The kernel floor is untouched and every kernel file still holds 100%, the write transaction included: this is dilution by test infrastructure, not eroded product coverage.
