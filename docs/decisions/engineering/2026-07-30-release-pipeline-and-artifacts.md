...
- Status: accepted (amended 2026-07-30 and 2026-07-31 — see [Amendments](#amendments))
- Date: 2026-07-30
...
- A tag push yields a complete draft prerelease with four attested archives, per-asset sidecars, and an aggregate `sha256.sum`; nothing reaches users without a human publish. (Amended 2026-07-31 — each archive now also carries a CycloneDX SBOM asset and a second attestation over it.)
...
- The pipeline has exactly five pinned third-party action SHAs (checkout, upload/download-artifact, rust-toolchain, attest-build-provenance) and dependabot watches all of them; everything else is first-party runner tooling or scripts in this repo. (Amended 2026-07-30 and 2026-07-31 — see [Amendments](#amendments).)
...
- **2026-07-30 — the release build is wrapped in `cargo auditable`.** `cargo build --release` became `cargo auditable build --release`, embedding the exact linked dependency list in a ~370-byte `.dep-v0` section so a shipped artifact can be audited directly with `cargo audit bin` rather than by trusting a lockfile elsewhere. Verified byte-identical across rebuilds, so the archive-determinism claim above is unaffected. Recorded in the [supply chain and vulnerability policy](2026-07-30-supply-chain-and-vulnerability-policy.md).
- **2026-07-30 — the pinned third-party action count is no longer five.** Adding `taiki-e/install-action` (for `cargo-auditable` and `git-cliff`) makes it seven distinct actions across the release path. The pinning rule is unchanged, and is now checked by zizmor rather than by review.
- **2026-07-30 — release notes are generated by git-cliff, not `--generate-notes`.** The draft body is built from the Conventional Commit history for the tag range. Draft-first and human publication are unchanged. Recorded in the [commit convention and release notes ADR](2026-07-30-commit-convention-and-release-notes.md).
- **2026-07-31 — the release path mints SBOM attestations, and the pinned action count is nine.** `actions/attest` (SHA-pinned) joins the release job, minting one CycloneDX attestation per target archive beside the build-provenance one; `actions/attest-sbom` is deliberately not used, upstream having deprecated it. The count is restated here with its scope named, because "seven distinct actions across the release path" fits two different measurements and matches neither now: across all three workflows the repository pins **nine** — `actions/checkout`, `actions/upload-artifact`, `actions/download-artifact`, `dtolnay/rust-toolchain`, `taiki-e/install-action`, `actions/attest-build-provenance`, `actions/attest`, `zizmorcore/zizmor-action`, and `google/osv-scanner-action`, whose two reusable workflows share one pin — of which the release workflow itself uses seven. `zizmorcore/zizmor-action` arrived with the gate workflows and was never counted, so this is a recount as much as an increment. The pinning rule is unchanged and zizmor enforces it. Recorded in [the M2 prerelease and cutover record](2026-07-31-m2-release-and-cutover.md).
