...
- **2026-08-08 — `born-flagged` takes a list of flag names, never a boolean.** The Decision seats a birth state on `[[type]]` without fixing its shape. Adjudicated with the implementation: the seat is `born-flagged = ["needs_triage"]`, naming flags the type itself declares. A bare boolean cannot work, because it would have to mean *the* triage flag, and the kernel binds to declarations rather than to a vocabulary — a contract's flags are its own, and no version of this format knows a privileged name. A list also states the polarity the Decision needs: an undeclared birth state stamps nothing, and an empty list says the same thing explicitly. Each named flag must be declared by the type and declared as a flag, which `contract.birth-flag-undeclared` and `contract.birth-flag-not-a-flag` enforce.
- **2026-08-08 — the default capture directory is `captures`.** The seat's default, where a contract declares no `[capture]`. Named for the act rather than borrowed from the inbox-workflow PDR's `inbox`: the PDR's inbox is the *unfiled state* a capture lands in, which this format already expresses through the catch-all binding, and reusing the word for a directory would make a mechanic the Decision says carries no meaning look like the state that does. The consequence this record already accepted stands — the default becomes de-facto vocabulary — and `captures` is the vocabulary it becomes.
