...
- Status: accepted (amended 2026-08-01, 2026-08-04 — see [Amendments](#amendments))
- Date: 2026-07-31
...
**"Above the range" includes a version no `u32` holds.** The domain is every whole number 0 or above and has no upper bound, so classification is total over it only if a literal too large to represent is *above every supported range, whatever the range is* — which it is. Such a version therefore takes `compat.contract-too-new` (or `compat.installation-too-new`) and travels through the message as the file's own bytes, not as a number it is not. The JSON keeps `version.found` `u32`-bounded on purpose and reports `null` for a version it cannot hold, so the TypeScript binding never sees an integer past `2^53` where it would silently lose precision; the classification is still reported, which is the fact a consumer acts on.
...
- **2026-08-01 — `doctor` never refuses one step earlier than the Decision says, either.** The Decision states that `dogtag doctor` never refuses to run and illustrates it with a contract outside the supported range. A selection that resolved *no vault* was outside that illustration and was implemented as a refusal: nothing on standard output in either format, so a `--format json` consumer received an empty stream and an exit code. That is the run during which the parallel-run triage most needs a document — an unregistered name and a missing record are the two states a reader cannot diagnose from an exit code. `doctor` now reports what is known whatever happened: the selection that was attempted, the installation record's state, version and actor, and every vault-dependent section saying *not evaluated* with the reason. `vault.root` is `null` rather than absent, so the JSON has one shape either way. The exit code is unchanged, and severity still decides it.

- **2026-08-01 — the in-vault-path rule is held by construction rather than by call-site discipline.** The Decision makes in-vault paths vault-relative and forward-slashed, and the reasoning above says why: an absolute path in structured output makes a conformance golden machine-specific, and the rule doubles as a privacy control. `FileRef::InVault` nevertheless carried a public `String`, so any spelling a caller could write — an absolute path, a Windows-separated one, a path out of a different vault — reached the rendering and the JSON, and the only function that produced a correct payload, `VaultRoot::relative`, had no production caller at all. The payload is now a `VaultPath` whose two constructors are both crate-internal: stripping a real root's prefix off a path, and a `&'static str` for the kernel's own committed constant. `VaultRoot::relative` is the public door, and the only way a consumer reaches `FileRef::InVault` — which means holding the root the path is under. **There is deliberately no public shape-validating constructor**: a check that a string looks vault-relative would pass any string that looks it, and what is wanted is that no caller-supplied runtime string reaches output through this type at all. Two limits are worth stating so nobody reads more into it than it holds: it does not prove the file exists, since nothing here touches the filesystem, and it does not prove the path belongs to the vault a report is about — a `VaultPath` does not remember which root it was stripped against, so one taken against root A can still be attached to a diagnostic about root B.

- **2026-08-04 — the quoting license extends to a note's own value and reference text.** The Decision licenses quoting "type names, property names, lifecycle values" — schema vocabulary — and M3's validators want to name the value that failed its kind and the reference that resolved to nothing, which is note *content* outside that enumeration. Extended deliberately: a diagnostic message may quote the offending value or reference bytes, under the standing disciplines — prefer the location where it alone identifies the problem, every control character folds before a terminal, and the practical rule stands that **diagnostic output from a private vault is private**, which matters more, not less, now that messages can carry note content into a pasted issue.
