...
The Decision above stands as written; this later entry changes its evidence duration, and the original text is left intact so the change is legible.

- **2026-08-12 — the cutover closes after five days because its evidence goal is met; seven days was a heuristic, not a demonstrated threshold.** Max adjudicated that waiting until August 14 solely to satisfy elapsed time would add no known evidence. Since day one, real search and resolution paths used the installed binary; the incumbent mechanisms were verified; behavioral differences and residue were classified; founder-vault latency was measured; and no Dogtag defect or return to the incumbent occurred outside the named residue. The durable beta rule is that a real workflow moves and does not move back; it specifies no duration. Rejected: treating the two remaining calendar days as intrinsically valuable without a query-count, usage-diversity, or defect-discovery claim they would satisfy. This is not a new five-day rule. M6's release record replaces calendar-shaped cutover evidence with explicit observations.
- **2026-08-12 — M4 closes and the local incumbents retire.** `dogtag search` and `dogtag find` remain the authority for the repointed daily and skill paths. The local `pkm search`, `pkm find`, and reindex scheduling retire for the behavior Dogtag now owns. Three residues remain by name: the connector's server-side FTS database and `pkm tags` go to M6; fuzzy, phonetic, context-ranked, batch, and richer-filter resolution remain with the incumbent until a milestone schedules them; and the `just search` fzf picker goes to the TUI milestone. The founder-vault medians are 650/714 ms against a roughly 450 ms corpus-load floor, below the one-second index trigger and close enough that M6 must measure again.
