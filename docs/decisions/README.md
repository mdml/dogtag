...
Product decisions recorded the way [ADRs](../adr/README.md) record technical ones — but for *what the product is*, not *how it's built*. This directory holds one file per decision; the [six-document set](../../README.md) at the repository root carries the product thesis the decisions hang off.
...
- **MCP** — …
- **CLI / SDK** — …
- **Tolaria (GUI fallback)** — … (omit a surface if it genuinely doesn't apply)
```
...
- **Product *concepts*** that don't have a home yet — the substrate/surfaces model, the config/invariant framing, "the index" as a product idea — live in the root doc set ([PRODUCT](../../PRODUCT.md), [ABSTRACTIONS](../../ABSTRACTIONS.md)) or in a *foundational* PDR, and other PDRs reference that.
