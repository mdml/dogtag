...
- **M6 — TypeScript SDK and dynamic workflow.** The napi-rs binding backed by the Rust core, published to npm, then an SDK-backed MCP server (`search`, `show`, `capture`, `tags`) deployed alongside the maintainers' existing infrastructure. `tags` discharges the named M4 residue; the surface remains bounded rather than mirroring the whole SDK. Capture has no real incumbent to cut over — M6 establishes its first workflow and closes M5's rolled-forward obligation with an honest no-incumbent receipt. *(Decided 2026-08-12 in [the M6 packet](decisions/engineering/2026-08-12-m6-fixtures-release-and-acceptance.md).)*
