...
The first participant is already identified and already operates a sophisticated non-Dogtag knowledge system of their own. That combination makes them an E1 subject rather than an E0 maintainer: E0 tests whether founders can run the product on vaults they built for it, and adoption of an existing corpus is a materially different question. Folding them into E0 would fuse the two, so that a failure could not be attributed to either the SDK or the guided-adoption process. The `records` fixture profile exists so that assisted onboarding of an established corpus is rehearsed against a fixture before it is attempted against a real one.
