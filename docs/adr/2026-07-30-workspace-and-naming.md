...
- One quirk to remember at first publish: a plain `"0.1"` requirement will not match a prerelease, so early consumers of the crate must require `=0.1.0-beta.0` (or the then-current prerelease) explicitly — documented here so it lands in the publish checklist rather than as a surprise.
