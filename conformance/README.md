...
Two input shapes are **not** contract transformations, and the rule says so rather than implying a coverage it does not have. The machine-local installation record has no per-profile source, so its cases run one identical input against every profile; the per-profile half of that scenario opens the profile's **own** vault with a smuggling record in scope and asserts that the contract that vault resolves — every declaration, and every leaf's attribution — is the contract the same vault resolves with no record at all, having first shown the record was genuinely in scope by finding its refusal in that vault's diagnostic list. `open` never passes the installation record into contract resolution, so as the SDK is shaped today that comparison cannot fail; it is a regression guard sited where the failure would appear, and it says so in the case rather than reading as an observation. It replaced an assertion that called `clean_contract()` on a vault no record had ever been opened against — where "unaffected" held by the harness's own filesystem construction, making the case an unlabelled second copy of `conforming-contract-loads-with-zero-diagnostics`. The three discovery scenarios need a synthetic directory *tree*, where the profile contributes a contract the sentinel test never parses, so what varies between profiles there is nothing. **Those are one case each, not four**, and the printed cross product should not be read as four.

### A profile's situation is derived too, never scoped to that profile

The same rule answers what a scenario written for one fixture's distinguishing axes can mean here. Four M4 scenarios were drafted as `docs`-only — a recurring basename, a reference in the markdown dialect, a note with no frontmatter at all — and scoping them to `docs` is precisely what the schema has no field to express. So they derive instead: each plants its situation into a fresh copy of whichever corpus it runs against, and the one that turns on the link dialect writes its references in **whichever dialect that corpus declares** and its counter-example in the other. What "docs-only" names after that is the corpus that exhibits those situations in its *committed* form, which is a claim about a fixture rather than about a scenario — and [`harness/tests/floors.rs`](harness/tests/floors.rs) is where it is asserted, of `docs` alone. See [the M4 fixtures record's amendments](../docs/decisions/engineering/2026-08-06-m4-fixtures-and-conformance.md#amendments).
...
5. every scenario is `executable` — nothing is pending anywhere — so a straggler fails the suite rather than sitting out a graduation;
